...
The simulated data set `ecvs` in __mecor__ is a external covariate-validation study. An external validation study is used when in the main study, no information is avaible to correct for the measurement error in the exposure $X$. Suppose for example that  $X$ is not observed in the internal covariate-validation study `icvs`. An external validation study is a (small) substudy containing observations of the reference measure $X$, the error-prone substitute measure $X^*$ and the covariate $Z$ of the original study. The external validation study is then used to estimate the calibration model, that is subsequently used to correct for the measurement error in the main study. 
